...
def parse_instruction_line(line: str) -> Tuple[float, str]:
    ...
    try:
        ...
        logging.getLogger(__name__).warning("Error parsing line: '%s' - %s", line, e)
        return None, None
...
class TextInstructions(QObject):
    ...
    def __init__(
        ...
        self.instructions = []
        try:
            with open(self.instruction_file, encoding="utf-8") as file:
                for line_num, line in enumerate(file, 1):
                    ...
                    if duration is not None and text is not None:
                        ...
                        logging.getLogger(__name__).warning(
                            "Skipped malformed line %s: '%s'", line_num, line.strip()
                        )
        except Exception as e:
            import logging

            logging.getLogger(__name__).exception(
                "Error reading file '%s': %s", self.instruction_file, e
            )
            return

        try:
            with open(self.instruction_file) as fobj:
                self.instructions = fobj.read().splitlines()
        except Exception:
            return
